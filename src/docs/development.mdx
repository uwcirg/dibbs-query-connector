...
The [docker-compose.complete.yaml](../../docker-compose.complete.yaml) file runs Keycloak, Aidbox, PostgreSQL, Flyway, and the Query Connector app entirely in containers. It includes [docker-compose-dev.yaml](../../docker-compose-dev.yaml) plus [docker-compose.complete.overrides.yaml](../../docker-compose.complete.overrides.yaml) (required by Compose so included services are not redefined in the same file), and adds the app service from the e2e compose file.
...
##### Traefik ingress overlay

To expose services via Traefik on the external `external_web` network, merge [docker-compose.ingress.yaml](../../docker-compose.ingress.yaml):

1. Ensure Traefik is running on `external_web` with a `websecure` entrypoint and `letsencrypt` cert resolver.
2. Create the network if needed: `docker network create external_web`
3. Set `BASE_DOMAIN` in `.env` to your DNS zone (e.g. `dev.example.com`).
4. Start: `docker compose -f docker-compose.complete.yaml -f docker-compose.ingress.yaml up -d --build`
...
Host port bindings are removed when the ingress overlay is used; access is via Traefik only.
