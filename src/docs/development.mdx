...
#### Running with layered Docker Compose (Traefik)

For a full stack (Query Connector, Keycloak, Aidbox, Postgres) behind HTTPS with hostnames like `https://localtest.me`, use [`opt/dibbs-environments`](../../opt/dibbs-environments/README.md):

1. Start Traefik: `cd opt/dibbs-environments/extras && docker compose up --detach`
2. Follow setup in [`opt/dibbs-environments/dev/README.md`](../../opt/dibbs-environments/dev/README.md)

This path uses Traefik and `*.localtest.me` DNS instead of direct host ports.
