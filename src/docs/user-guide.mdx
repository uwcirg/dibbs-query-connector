...
Query Connector currently supports the following five authentication methods:

1. Provide a valid Auth Bearer token that Query Connector sends along with our FHIR requests
2. Provide a valid base64-encoded credential for Basic (username-password) auth, that Query Connector sends along with our FHIR Requests
3. Standard client\_credentials OAuth flow where you provide a client ID and client secret
4. SMART on FHIR with asymmetric key exchange
5. Mutual TLS client certificate flow, using existing configured certificate/keys or a provided CA certificate if the server is untrusted
